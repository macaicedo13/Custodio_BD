/* =============================================================================
  ...
  Objetos desplegados (en orden de dependencias) :
    ...
    01. Logins SQL Server: svc_caducidad
    02. Usuarios en dbdba: svc_caducidad
...
CREATE OR ALTER PROCEDURE noprod.SP_ETIQUETAR_BASES
    ...
    @FechaExpira    DATE          = NULL,
    @Debug          BIT           = 0,
	@AllowPast      BIT  = 0    -- 1 = permite fechas pasadas (solo laboratorio)
AS
BEGIN
    SET NOCOUNT ON;
    SET XACT_ABORT ON;
...
----------------------------------------------------------------------------- */
    ...
    IF @Razon = 'RSE' AND @FechaExpira < CAST(GETDATE() AS DATE) AND @AllowPast = 0
...
WHERE name IN ('svc_caducidad');

PRINT '';
PRINT '=== Despliegue CUSTODIO v1.9 completado ===';
PRINT '';
PRINT 'PENDIENTE MANUAL:';
PRINT '  1. Cambiar password de svc_caducidad (<CAMBIAR_PASSWORD>).';
